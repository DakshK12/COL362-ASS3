...
void merge(int start_idx , int end_idx , int stage , int stage_count , int Total_memory ){
...
int merge_runs(int num_runs , int max_fanout , string outfile , int stage , int totmem ){
    ...
    while(start < num_runs){
        if( (start + max_fanout - 1 ) < num_runs) {
            merge(start , start + max_fanout -1 , stage , withtin_stage_count , totmem);
            ans ++;
            start += max_fanout;
            withtin_stage_count ++ ;
        }
        else{
            merge(start , num_runs - 1 , stage , withtin_stage_count , totmem);
            ans ++;
            start = num_runs ;
            withtin_stage_count ++ ;
        }
    ...
    else return (ans + merge_runs(withtin_stage_count -1 , max_fanout , outfile , stage + 1, totmem)); 
...
int external_merge_sort_withstop(const char* input, const char* output, const long keycount , const int k, const int num_merges){
    ...
    int anss = merge_runs(runs_count , k , output , 0 , 50000000*3 );

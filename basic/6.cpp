...
int merge_runs(int num_runs , int max_fanout , string outfile , int stage , int totmem , int num_merges_allowed , int num_merges_done ){
    ...
    while(start < num_runs){
        if( (start + max_fanout - 1 ) < num_runs) {
            if ( (start + max_fanout == num_runs) && withtin_stage_count == 1 ) merge(start , start + max_fanout -1 , stage , withtin_stage_count , totmem , outfile , true );
            else merge(start , start + max_fanout -1 , stage , withtin_stage_count , totmem , outfile , false );
            num_merges_done ++;
            start += max_fanout;
            withtin_stage_count ++ ;
        }
...
int external_merge_sort_withstop(const char* input, const char* output, const long keycount , const int k, const int num_merges){
    ...
    int runs_count = generate_runs(input, 50000000*20 , keycount); //5*1e7 is 50 mb  
